...
- Refined the spacing logic so the gap is skipped whenever another lane already occupies the new block’s start time—this keeps overlap (e.g., “Katzne” vs. “Beispiele”) accurate without reintroducing touching cards for true back-to-back sequences.
- Added an 8 pt base overlap buffer plus a dynamic boost that kicks in when the timeline compresses (points-per-minute < 1), so cards like “Katzne” stay slightly below “Test 2” even after more events are added while still lining up beside overlapping neighbors.
